...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchApplications() {
        ...
        fetch('<?= base_url('/jobs/display') ?>') // Ensure this URL matches your routes
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    if (applications.length === 0) {
                        candidateListings.innerHTML = "<p>No applications found.</p>";
                    } else {
                                ...
                                <div class="col-md-4">
                                    <div class="candidate-card position-relative p-3 text-center">
                                        ...
                                        <div class="candidate-avatar mx-auto mb-3">
                                            <img src="${baseURL}${application.profile_image}" 
                                                 alt="${application.candidate_name}" 
                                                 class="img-fluid">
                                        </div>
                                        <h4 class="candidate-title">${application.candidate_name}</h4>
                                        <p>${application.job_type}</p>
                                        <p><i class="fa fa-map-marker"></i> ${application.job_location}</p>
                                        <a href="<?= base_url('/job/profile') ?>?id=${application.application_id}" 
                    ...
                    candidateListings.innerHTML = "<p>Error loading applications. Please try again later.</p>";
                }
            })
            .catch(error => {
